...
export class LLMService {
     ...
     * @returns 分析结果
     */
    async analyzeTask(description, context, analysisType) {
     ...
     * @returns 推荐结果
     */
    async recommendTools(task, availableTools) {
        try {
     ...
     * 优化版本：更简洁明确的提示，更快的响应
     */
    generateTaskAnalysisPrompt(description, context, analysisType) {
        // 决定分析深度
...
输出格式:
{
  ...
  "domain": "领域"
     ...
     * 创建任务分析提示词
     */
    createTaskAnalysisPrompt(description, context) {
        return `
...
`;
     ...
     * 优化版本：更简洁明确的工具推荐提示
     */
    createToolRecommendationPrompt(task, availableTools) {
        // 格式化工具列表，突出重点信息
...
`;
     ...
     * 解析工具推荐响应
     */
    parseToolRecommendationResponse(response) {
        try {
            // 先尝试解析JSON
            let jsonStr = response.trim();
            // 查找可能的JSON开始位置（跳过可能的前导文本）
     ...
     * 从多行文本中提取推荐理由
     */
    extractToolsReasoning(text, pattern) {
        const match = text.match(pattern);
     ...
     * 获取模拟任务分析结果（用于开发模式）
     */
    getMockTaskAnalysis(description) {
    ...
    async testAPIConnection() {
        try {
            ...
            if (response.ok) {
                ...
                // 检查配置的模型是否在可用列表中
                const modelExists = data.data.some((model) => model.id === this.config.modelName);
                if (modelExists) {
