...
export class MCPAssistantServer {
    ...
    // 初始化LLM服务
    async initializeLLMService(config) {
        try {
    ...
    // 处理分析任务请求
    async handleAnalyzeTask(params) {
    ...
    // 处理推荐工具请求
    async handleRecommendTools(params) {
    ...
    // 处理更新上下文请求
    async handleUpdateContext(params) {
    ...
    // 处理发现MCP工具请求
    async handleDiscoverMCPTools(params) {
    ...
    // 处理调用外部工具请求
    async handleCallExternalTool(params) {
        ...
        if (this.serviceNameMap.has(originalServiceName)) {
            serviceName = this.serviceNameMap.get(originalServiceName);
