...
// 预定义的MCP工具列表
const PREDEFINED_TOOLS: MCPTool[] = [
  {
    name: 'file_read',
    description: '读取文件内容',
...
export class DefaultToolRecommender implements ToolRecommender {
  ...
  async recommendTools(task: Task, context: Context): Promise<ToolRecommendation> {
    // 获取可用工具列表
  ...
  async matchToolCapabilities(task: Task): Promise<MCPTool[]> {
    // 本地基于关键词和任务类型匹配工具
    const keywordMatches = new Set<MCPTool>();
    ...
    if (task.entities) {
      for (const entity of task.entities) {
        for (const tool of PREDEFINED_TOOLS) {
          if (
            tool.description.includes(entity) ||
    ...
    // 根据任务类型匹配
    const typeMatches = new Set<MCPTool>();
    switch (task.type) {
      case 'FileOperation':
        PREDEFINED_TOOLS.filter(t => 
          t.name.includes('file') || 
          t.capabilities.some(c => c.includes('文件'))
        ).forEach(t => typeMatches.add(t));
        break;
      case 'CodeAnalysis':
        PREDEFINED_TOOLS.filter(t => 
          t.name.includes('code') || 
          t.capabilities.some(c => c.includes('代码'))
        ).forEach(t => typeMatches.add(t));
        break;
      case 'SystemOperation':
        PREDEFINED_TOOLS.filter(t => 
          t.name.includes('terminal') || 
          t.capabilities.some(c => c.includes('系统') || c.includes('命令'))
        ).forEach(t => typeMatches.add(t));
        break;
      case 'DataProcessing':
        PREDEFINED_TOOLS.filter(t => 
          t.name.includes('data') || 
          t.capabilities.some(c => c.includes('数据'))
        ).forEach(t => typeMatches.add(t));
        break;
      case 'WebSearch':
        PREDEFINED_TOOLS.filter(t => 
          t.name.includes('web') || t.name.includes('search') ||
          t.capabilities.some(c => c.includes('搜索') || c.includes('网络'))
        ).forEach(t => typeMatches.add(t));
        break;
      case 'DatabaseOperation':
        PREDEFINED_TOOLS.filter(t => 
          t.name.includes('database') || 
          t.capabilities.some(c => c.includes('数据库') || c.includes('SQL'))
        ).forEach(t => typeMatches.add(t));
        break;
      case 'APIInteraction':
        PREDEFINED_TOOLS.filter(t => 
          t.name.includes('api') || 
          t.capabilities.some(c => c.includes('API') || c.includes('服务'))
        ).forEach(t => typeMatches.add(t));
        break;
      case 'DocumentProcess':
        PREDEFINED_TOOLS.filter(t => 
          t.name.includes('document') || 
          t.capabilities.some(c => c.includes('文档') || c.includes('文本'))
        ).forEach(t => typeMatches.add(t));
        break;
      case 'ImageProcess':
        PREDEFINED_TOOLS.filter(t => 
          t.name.includes('image') || 
          t.capabilities.some(c => c.includes('图像') || c.includes('视觉'))
        ).forEach(t => typeMatches.add(t));
        break;
    }
  ...
  async optimizeSequence(task: Task, tools: MCPTool[], context: Context): Promise<string[]> {
    // 获取工具名称
    ...
    switch (task.type) {
      case 'FileOperation':
        // 对于文件操作，通常先读取再写入
        const readTools = toolNames.filter(t => 
          t.includes('read') || t.includes('search') || t.includes('analysis')
        );
        const writeTools = toolNames.filter(t => 
          t.includes('write') || t.includes('create') || t.includes('edit')
        );
        const otherFileTools = toolNames.filter(t => 
          !readTools.includes(t) && !writeTools.includes(t)
        );
        return [...readTools, ...otherFileTools, ...writeTools];
      
      case 'DataProcessing':
        // 对于数据处理，通常先获取数据再转换
        const fetchTools = toolNames.filter(t => 
          t.includes('fetch') || t.includes('get') || t.includes('read')
        );
        const transformTools = toolNames.filter(t => 
          t.includes('transform') || t.includes('process') || t.includes('convert')
        );
        const otherDataTools = toolNames.filter(t => 
          !fetchTools.includes(t) && !transformTools.includes(t)
        );
        return [...fetchTools, ...transformTools, ...otherDataTools];
      
      case 'WebSearch':
        // 对于网络搜索，先搜索再处理
        const searchTools = toolNames.filter(t => 
          t.includes('search') || t.includes('fetch')
        );
        const processTools = toolNames.filter(t => 
          t.includes('process') || t.includes('analyze')
        );
        const otherWebTools = toolNames.filter(t => 
          !searchTools.includes(t) && !processTools.includes(t)
        );
      ...
      default:
        ...
        const analyzeTools = toolNames.filter(t => 
          t.includes('analysis') || t.includes('check')
        );
        const acquireTools = toolNames.filter(t => 
          (t.includes('get') || t.includes('fetch') || t.includes('read')) && 
          !analyzeTools.includes(t)
        );
        const processTools2 = toolNames.filter(t => 
          (t.includes('process') || t.includes('transform')) && 
          !analyzeTools.includes(t) && !acquireTools.includes(t)
        );
        const executeTools = toolNames.filter(t => 
          !analyzeTools.includes(t) && !acquireTools.includes(t) && !processTools2.includes(t)
        );
