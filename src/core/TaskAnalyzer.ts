...
export class DefaultTaskAnalyzer implements TaskAnalyzer {
  ...
  // 分析任务，返回结构化数据
  async analyzeTask(description: string, context?: string): Promise<Task> {
    // 尝试使用大模型分析
  ...
  // 确定任务复杂度（1-5）
  async determineComplexity(description: string): Promise<number> {
    // 简单的复杂度评估逻辑
    // 统计描述长度、子句数量、专业词汇等指标
  ...
  // 分类任务类型
  async classifyTaskType(description: string): Promise<TaskType> {
    // 简单的任务类型分类逻辑
    const typePatterns = [
  ...
  // 识别任务中的实体
  async identifyEntities(description: string): Promise<string[]> {
    // 简单的实体识别逻辑
  ...
  // 将任务分解为子任务
  async splitIntoSubTasks(description: string): Promise<string[]> {
    // 简单的子任务分解逻辑
  ...
  // 确定任务优先级（1-5）
  async determinePriority(description: string): Promise<number> {
    // 简单的优先级确定逻辑
    ...
    if (hasUrgentKeywords) priority += 2;
    ...
    if (hasLowPriorityKeywords) priority -= 2;
  ...
  // 估计任务完成时间（秒）
  async estimateTaskDuration(description: string, complexity: number): Promise<number> {
    // 简单的任务时间估计逻辑
  ...
  // 识别任务领域
  async identifyDomain(description: string): Promise<string> {
    // 简单的领域识别逻辑
  ...
  private mapLLMTaskType(llmTaskType: string): TaskType {
    // 尝试直接映射
